...
export const getConsolidated58mmPayload = (
    ...
    if (data.estimationItems.length > 0) {
        data.estimationItems.forEach(item => {
            ...
            const amountStr = formatCurrency(item.totalValue); // This is net amount pre-GST in this context usually, but we need to ensure clarity
        ...
        payload += thermalRow('Items Value', formatCurrency(data.totals.estimationTotal), width);
